...
*** What is this?
    ...
    1. Some of the files contain configuration in a language called
    ...
    2. Other files contain HTML and serve as the basis of a simple
       "website:" https://jedcn.github.io/mac-config.
...
*** Wait, what's =.org=?
    ...
    *This* is a file named =README.org=. That extension, =.org=, means
    ...
    such) side-by-side with my boxen configuration.
...
** Configuration
   ...
   This section details my actual configuration.

   We'll start with prose alongside puppet, and later on I will
   explain how everything comes together in the Administrativia
   section.

   All of the configuration that you're about to see is functional. It
   defines "rules" about how my computer should be configured. This
   includes:

   + Which system settings should be in place
   + Which repositories should be cloned (say, dotfiles)
   + Which links should be created (saa, to those cloned repositories)
   ...
   so, if there's a rule that says 'Chrome needs to be installed' then
   puppet will check to see if Chrome is present, and install it if it
   is not. *If* Chrome already is installed, then puppet does nothing.

   This means that a first run of boxen might take 30 or 40 minutes
   while dozens of programs are downloaded and installed, but
