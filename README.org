...
*** Applications
    ...
    we figure out applications like Chrome and Hammerspoon.

    You can use boxen to install the applications that you work
    with. In turn, boxen relies on modules to install
    applications. Originally, I used modules like this:

    * [[https://github.com/boxen/puppet-chrome][puppet-chrome]]

    But this did not work out well. Modules were hit-or-miss. Some
    were not updated. This hardship caused me to try out brewcask, and
    so far as I can tell, that is the way to go with boxen.

    Whether called out with an explicit puppet module, like
    puppet-chrome, or called out via brewcask, after you declare that
    you want to have an application, say chrome, you run the command
    =boxen= and the application will "just" appear in your
    Applications folder.

    This allows you to replace visiting a website, downloading a
    =.dmg= file, and running an installer with writing a few lines of
    configuration and running boxen.
...
**** Boxen vs. Manual Install
     ...
     boxen, that's what I'd prefer. And when I'm using boxen, I
     strongly lean toward using brewcask.
...
**** Window Management
     ...
     I use Hammerspoon for window management, among other things.

     After you run boxen, I still launched Hammerspoon and did the
     following manually:
...
**** Declarations
     ...
     + [[https://github.com/JohnCoates/Aerial][Aerial]] is a Screen Saver. Boxen will install it, but you still
       need to Open System Preferences -> Desktop & Screen Saver ->
       Screen Saver, and set it up.
...
**** =rakelib/ghpages.rake=
     ...
     This static content will be hosted on Github Pages.
     ...
     To be consistent, I should've set up something that knows about
     downloading primer.
...
*** Which version do you use for a module?
    ...
    https://github.com/boxen/puppet-chrome, you can add it to your
    =Puppetfile=.
